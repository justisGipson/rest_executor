// caller API can validate if the user has access to the server or not
...
pub fn authenticate(username: &str, password: &str) -> bool {
  let mut authenticator = Authenticator::with_password("login").expect("Fail to init with client");
